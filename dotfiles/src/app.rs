...
pub struct App {
    ...
    pub running: bool,
    /// Applications to install + references to their dependencies
    pub dependencies: Vec<Rc<dyn Dependency>>,
    /// How the graph of dependencies is structured
    pub dependency_graph: Vec<Vec<Rc<dyn Dependency>>>,
}

impl Default for App {
    fn default() -> Self {
        let dep_a = TestDependency::new("dep_a");
        let dep_b = TestDependency::new("dep_b");
        let dep_c = TestDependency::new("dep_c");
        let dep_d = TestDependency::new("dep_d");
        let dep_e = TestDependency::new("dep_e");
        let dep_f = TestDependency::new("dep_f");

        dep_a.add_dependency(dep_b.clone());
        dep_a.add_dependency(dep_c.clone());
        dep_b.add_dependency(dep_d);
        dep_b.add_dependency(dep_e);
        dep_c.add_dependency(dep_f);

        let dependencies: Vec<Rc<dyn Dependency>> = vec![dep_a];
        // for each dependency, call is_enabled to propagate the enabled state
        ...
        Self {
            running: true,
            dependency_graph: build_dependency_graph(dependencies.clone()),
            dependencies,
        }
    }
}
...
impl std::fmt::Debug for TestDependency {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let parents = self.parents.borrow();
        let parents = parents
            .clone()
